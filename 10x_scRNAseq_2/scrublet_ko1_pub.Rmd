...
```{r prep object}
UMImatrix <- ko1[["RECODE"]]@counts#このマトリックスはcolnamesがcellbarcode、rownamesがgene symbolである。
genesymbolUMI <- rownames(ko1[["RECODE"]]@counts)
cellbarcodeUMI <- colnames(ko1[["RECODE"]]@counts)
```
...
scrub = scr.Scrublet(counts_matrix, expected_doublet_rate=0.005)
...
ko1 <- AddMetaData(ko1,py$predicted_doublets,"Is_doublets")
ko1 <- subset(ko1, subset = Is_doublets == "FALSE")
