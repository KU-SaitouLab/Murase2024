...
```{r prep object}
UMImatrix <- ko2[["RECODE"]]@counts#このマトリックスはcolnamesがcellbarcode、rownamesがgene symbolである。
genesymbolUMI <- rownames(ko2[["RECODE"]]@counts)
cellbarcodeUMI <- colnames(ko2[["RECODE"]]@counts)
```
...
scrub = scr.Scrublet(counts_matrix, expected_doublet_rate=0.009)
...
ko2 <- AddMetaData(ko2,py$predicted_doublets,"Is_doublets")
ko2 <- subset(ko2, subset = Is_doublets == "FALSE")
```